<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST") {
		if ($_SESSION['admin'] == 1) {
			$hash = $_POST["popis"];
			$sql = "update queue set note = '".$hash."' where name = '".$_GET['nazev']."'";
			//echo $sql;
...
<html>  
   <head>
      <?php if($navrat) { header('Location: queue.php');} ?>
   ...
   <body>   
		...
		<?php
		$sql = "SELECT name, note, od, do FROM queue where name = '".$_GET['nazev']."'";
		$result = $db->query($sql);
